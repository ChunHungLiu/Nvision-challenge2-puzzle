...
namespace cv
{
	...
	public:
		std::vector<Rotation> rotation_matrix_s;

		RotationMatrix(){}

		RotationMatrix(double angular_velocity_){
		...
		size_t size(){
			return rotation_matrix_s.size();
		}

		void create(double angular_velocity_ = 30.0){
			rotation_matrix_s.clear();
			Rotation v;
			for (double i = 0; i < 360.0; i += angular_velocity_){
				v.r = i*CV_PI / 180.0;
				v.a[0] = v.a[3] = cos(v.r);
				v.a[2] = sin(v.r);
				v.a[1] = -v.a[2];

				rotation_matrix_s.push_back(v);
			}
		}
	...
	private:
		...
		RotationMatrix rotation_matrixs_;
	...
	public:
		ChamferMatcher(double angularVelocity = 30.0, int _max_matches = 20, float _min_match_distance = 1.0, int _pad_x = 3,
			int _pad_y = 3, int _scales = 5, float _minScale = 0.6, float _maxScale = 1.6,
			float _orientation_weight = 0.5, float _truncate = 20) : rotation_matrixs_(angularVelocity)
		{
	...
	*/
	...
	int orientationChamferMatching(Mat& img, Mat& templ,
		std::vector<std::vector<Point> >& results, std::vector<float>& costs,
		double angularVelocity = 30.0,
		...
		double orientationWeight = 0.5, double truncate = 20)
	{
		...
		ChamferMatcher matcher_(angularVelocity, maxMatches, (float)minMatchDistance, padX, padY, scales,
